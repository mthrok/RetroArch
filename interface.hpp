...
void runloop_msg_queue_push_(const char *msg,
...
char* runloop_msg_queue_pull_(void)
{
   runloop_ctx_msg_info_t msg_info;
...
bool runloop_ctl_(enum runloop_ctl_state state, void *data)
{
 ...
 **/
int runloop_iterate_(unsigned *sleep_ms)
{
   unsigned i;
   ...
   for (i = 0; i < settings->input.max_users; i++)
   {
      if (!settings->input.analog_dpad_mode[i])
         continue;
      printf("pushing input to analog_dpad\n");
   ...
   for (i = 0; i < settings->input.max_users; i++)
   {
      if (!settings->input.analog_dpad_mode[i])
         continue;
      printf("popping input from analog_dpad\n");
...
public:
  ...
  int step(void) {
    unsigned sleep_ms = 0;
    int ret = runloop_iterate_(&sleep_ms);
    task_queue_ctl(TASK_QUEUE_CTL_CHECK, NULL);
    return ret;
  }
    ...
    for (size_t i = 0; i < settings_->input.max_users; ++i) {
      ...
      for (size_t j = 0; j < RARCH_BIND_LIST_END; ++j) {
        ...
        */
        ...
        // printf("    valid  : %d\n", bind_.valid);
        // printf("    id     : %d\n", bind_.id);
        printf("    joykey : %zu (%zu)\n", bind_.joykey, bind_.def_joykey);
        printf("    joyaxis: %u (%u)\n", bind_.joyaxis, bind_.def_joyaxis);
        // printf("    joykey_label : %s\n", bind[j].joykey_label);
        // printf("    joyaxis_label : %s\n", bind[j].joyaxis_label);
      }
    }
  };
